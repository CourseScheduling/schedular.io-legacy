...
CORE.sort.functions=function(c){return{type:"Starting Time",time:{mornings:function(b,a){for(var c=0,g=0,d=0,f=0,h=b.length;h--;)for(var k=b[h].times.length;k--&&-1!=b[h].times[k].day[0];)c+=b[h].times[k].startTime*b[h].times[k].day.length,d+=b[h].times[k].day.length;for(h=a.length;h--;)for(k=a[h].times.length;k--&&-1!=a[h].times[k].day[0];)g+=a[h].times[k].startTime*a[h].times[k].day.length,f+=a[h].times[k].day.length;return c/d-g/f},noon:function(b,a){for(var c=0,g=0,d=0,f=0,h=b.length;h--;)for(var k=
b[h].times.length;k--&&-1!=b[h].times[k].day[0];)c+=Math.abs((b[h].times[k].startTime-b[h].times[k].endTime)/2-720)*b[h].times[k].day.length,d+=b[h].times[k].day.length;for(h=a.length;h--;)for(k=a[h].times.length;k--&&-1!=a[h].times[k].day[0];)g+=Math.abs((a[h].times[k].startTime-a[h].times[k].endTime)/2-720)*a[h].times[k].day.length,f+=a[h].times[k].day.length;return g/f-c/d},evenings:function(b,a){for(var c=0,g=0,d=0,f=0,h=b.length;h--;)for(var k=b[h].times.length;k--&&-1!=b[h].times[k].day[0];)c+=
b[h].times[k].startTime*b[h].times[k].day.length,d+=b[h].times[k].day.length;for(h=a.length;h--;)for(k=a[h].times.length;k--&&-1!=a[h].times[k].day[0];)g+=a[h].times[k].startTime*a[h].times[k].day.length,f+=a[h].times[k].day.length;return g/f-c/d},timeOfDay:"mornings"},teacherRating:function(b,a){for(var e=0,g=0,d=0,f=0,h=b.length;h--;)e+=c.main.teacher.ratings.map[b[h].times[0].instructor],d++;for(h=a.length;h--;)g+=c.main.teacher.ratings.map[a[h].times[0].instructor],f++;return g/f-e/d},availability:function(b,
a){for(var e=0,g=0,d=b.length;d--;)var f=c.socket.seatMap[b[d].crn],e=e+(1E5*(0<f.m-f.e)-100*f.w+(f.m-f.e)*(0<f.m-f.e));for(d=a.length;d--;)f=c.socket.seatMap[a[d].crn],g+=1E5*(0<f.m-f.e)-100*f.w+(f.m-f.e)*(0<f.m-f.e);return g-e},blockAmount:function(c,a){return a.times.length-c.times.length}}}(CORE);
...
CORE.main.teacher.functions=function(c){return{generate:function(b,a){var e=b.getElementsByClassName("r-ratingContainer")[0],g=0,d=0;a.forEach(function(a,b,f){b=document.createElement("div");b.innerHTML+=['<div class="crn_',a.crn,'"style="float:left;border:1px solid #DDD;box-sizing:border-box;height:28px;margin-top:1px;padding-left:5px;padding-right:5px;line-height:28px;margin-right:10px;"><strong style="text-shadow:0 0px 1px #999" data-seatsCRN="',a.crn,'"></strong> seats left</div>','<strong style="color:'+
c.helper.color.getBackgroundColor(a.courseName)+'">',a.courseName,"</strong> ",a.section," ",a.times[0].instructor," <strong>",0<c.main.teacher.ratings.map[a.times[0].instructor]?c.main.teacher.ratings.map[a.times[0].instructor]:"","<strong><br>"].join("");e.appendChild(b);0<c.main.teacher.ratings.map[a.times[0].instructor]&&(g+=c.main.teacher.ratings.map[a.times[0].instructor],d++)});var f=document.createElement("div");f.className="r-averageRating";f.innerHTML=(g/d).toFixed(3);e.parentNode.insertBefore(f,
...
CORE.main.search=function(c){return{depther:function(b,a){if(0>a)c.scheduleOptions.all.push(b);else{var e=c.courseData.array,g=e[a].sections.length;a:for(;g--;){for(var d=b.length;d--;){if(b[d].courseName===e[a].sections[g].courseName&&b[d].campus!==e[a].sections[g].campus&&"Online"!==b[d].campus)continue a;for(var f=b[d].times.length;f--;)for(var h=e[a].sections[g].times.length;h--;){var k=b[d].times[f],l=e[a].sections[g].times[h];if(c.helper.time.sameDay(k.day,l.day)&&c.helper.time.inTime(k.startTime,
k.endTime,l.startTime,l.endTime))continue a}}0==a?c.scheduleOptions.all.push(b.concat([e[a].sections[g]])):c.main.search.depther(b.concat([e[a].sections[g]]),a-1)}}},init:function(b){setTimeout(function(){var a=(new Date).getTime();c.courseData.array[c.courseData.array.length-1].sections.forEach(function(a,b,d){c.main.search.depther([a],c.courseData.array.length-2)});console.log("Scheduling Took: "+(Date.now()-a)+"ms");c.scheduleOptions.current=c.scheduleOptions.all;b&&b()},50)}}}(CORE);
function firstToUpperCase(c){return c.substr(0,1).toUpperCase()+c.substr(1)}
CORE.view.schedule=function(c){return{makeBlock:function(b,a,e){if(-1==e)return document.createElement("div");var g=c.helper.element.createDiv({"class":"s-timeBlock"});c.helper.element.changeStyle(g,{top:[(a.startTime-480)/2,"px"].join(""),left:[71*((e+1)%7),"px"].join(""),height:[(a.endTime-a.startTime)/2,"px"].join(""),lineHeight:[(a.endTime-a.startTime-4)/2,"px"].join(""),color:c.helper.color.getTextColor(c.helper.color.getBackgroundColor(b.courseName)),backgroundColor:c.helper.color.getBackgroundColor(b.courseName)});
g.innerHTML=b.courseName;return g},generate:function(b){var a=document.getElementById("scheduleWrapTemplate").cloneNode(!0),e=[];a.id="";b.forEach(function(b,d,f){e.push(b.crn);b.times.forEach(function(e,d,f){e.day.forEach(function(f,l,m){f=c.view.schedule.makeBlock(b,e,f);f.setAttribute("data-timeIndex",d);c.view.toolTip.add(f,b);a.getElementsByClassName("s-timeBlockWrap")[0].appendChild(f)})})});a.getElementsByClassName("s-useMeButton")[0].onclick=function(){localStorage.currentSchedule=e.join(".");
...
5:"Saturday",6:"Sunday"},d=document.getElementById("t-toolTipCourseName");d.innerHTML=[a.courseName,a.lab?" - LAB":"",'<label style="float:right;">',a.section,"</label>"].join("");d.style.color=c.helper.color.getBackgroundColor(a.courseName);var f=document.getElementById("t-toolTipCRN"),h=document.getElementById("t-toolTipInstructor"),d=document.getElementById("t-toolTipRating"),k=document.getElementById("t-toolTipImage");f.innerHTML="CRN: "+a.crn;h.innerHTML=a.times[e].instructor;0<c.main.teacher.ratings.map[a.times[e].instructor]?
(k.style.background="",f=c.main.teacher.ratings.map[a.times[e].instructor],d.innerHTML=f.toFixed(3),f>=TEACHER_RATING_AVERAGE+STANDARD_DEVIATION/2?k.style.backgroundColor="#2ecc71":f<TEACHER_RATING_AVERAGE+STANDARD_DEVIATION/2&&f>=TEACHER_RATING_AVERAGE-STANDARD_DEVIATION/2?(k.style.backgroundPosition="-100px 0px",k.style.backgroundColor="#f1c40f"):TEACHER_RATING_AVERAGE-STANDARD_DEVIATION/2&&(k.style.backgroundPosition="-200px 0px",k.style.backgroundColor="#e74c3c")):(d.innerHTML="N/A",k.style.background=
"#DDD");for(d=0;d<a.times.length;d++)for(k=0;k<a.times[d].day.length&&-1!==a.times[e].day[k];k++)document.getElementById("t-toolTipTimes").innerHTML+=["<strong>",g[a.times[d].day[k]],": </strong>",c.helper.time.getTime(a.times[d].startTime)," - ",c.helper.time.getTime(a.times[d].endTime),"<br>"].join("")},onMove:function(b,a){document.getElementById("t-toolTipWrap").style.display="none"},add:function(b,a){var c=this;b.addEventListener("mouseover",function(b){c.onHover(b,a)});b.addEventListener("mouseout",
